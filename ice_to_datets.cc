    ...
    FND* node,FunctionContext* ctx)
{
    ...
    *dest = seq_search((char*)*src, array, type, max, &len);
    if (len <= 0) {
        ...
        ctx->SetError("ERRCODE_INVALID_DATETIME_FORMAT. The given value did not match any of the allowed values for this field");
    }
    *src += len;
    return len;
}
...
try{
    ...
    if (tmfc.clock == CLOCK_12_HOUR) {
        if (tm.tm_hour < 1 || tm.tm_hour > HOURS_PER_DAY / 2) {
            context->SetError("hour is invalid for the 12-hour clock. Use 24-hour clock,or give an hour between 1 and 12.");
            return TimestampVal::null();
        }
    ...
    if (tmfc.ddd && (tm.tm_mon <= 1 || tm.tm_mday <= 1)) {
        if (!tm.tm_year && !tmfc.bc) {
            context->SetError("ERRCODE_INVALID_DATETIME_FORMAT.cannot calculate day of year without year information.");
            return TimestampVal::null();
        }
        if (tmfc.mode == FCDI) {
            int j0;
    ...
    const StringVal& formatStr)
{
  ...
  try{
    ...
    if(tmfc.ms > 999 || tmfc.us > 999999)
        {context->SetError("ERRCODE_INVALID_DATETIME_FORMAT.Invalid Time");return TimestampVal::null() ; }
   ...
   if ((tm.tm_hour > 24 || tm.tm_hour < 0) || (tm.tm_min > 59 || tm.tm_min < 0) || (tm.tm_sec > 59 || tm.tm_sec < 0))
       {context->SetError("ERRCODE_INVALID_DATETIME_FORMAT.Invalid Time");return TimestampVal::null() ; }
