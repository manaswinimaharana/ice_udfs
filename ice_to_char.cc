...
{   try
{
   ...
   if ( ifmt != NULL ) {    
    for (localfmt = ifmt; localfmt->type != N_END; localfmt++) {
        ...
        case ICE_HH12:
            tss.str("");
            (S_FM(localfmt->suffix)) ? timef->time_duration_format("%l") : timef->time_duration_format("%I");
            tss << pt.time_of_day();
        ...
        case ICE_HH24:
            tss.str("");
            (S_FM(localfmt->suffix)) ? timef->time_duration_format("%k") : timef->time_duration_format("%H");
            tss << pt.time_of_day();
            result.append(tss.str());
            tss.str("");
            break;
        ...
        case ICE_MI:
            tss.str("");
            (S_FM(localfmt->suffix)) ? timef->time_duration_format("%M") : timef->time_duration_format("%M");
            tss << pt.time_of_day();
            result.append(tss.str());
            tss.str("");
            break;
        ...
        case ICE_SS:
            tss.str("");
            (S_FM(localfmt->suffix)) ? timef->time_duration_format("%S") : timef->time_duration_format("%S");
            tss << pt.time_of_day();
            result.append(tss.str());
            tss.str("");
            break;
        ...
        case ICE_MM:
            oss.str("");
            (S_FM(localfmt->suffix)) ? datef->format("%m") : datef->format("%m");
            oss << pt.date();
            result.append(oss.str());
            break;
...
try {
   // FND* formatNd;
    ...
    if (formatNd == NULL ) {
        context->SetError("format string is malformed");
        return StringVal::null();
